...
import { LeavesTrail, MAX_STRING_DIST_SQ, TrailType, } from './components/leavesTrail';
...
export const createRenderSystem = (scene, objectManager) => {
    return defineSystem((world) => {
        ...
        for (const eid of updatedTrails) {
            ...
            if (trailType === TrailType.BEADS ||
                ...
                if (trailType === TrailType.MANY_BEADS) {
                    const lastChild = objectManager.getLastChild(eid);
                    if (lastChild) {
                        const bead = lastChild;
                        const dx = x - bead.x;
                        const dy = y - bead.y;
                        const stepX = dx / 4; // 2 extra circles => divide by 3
                        const stepY = dy / 4;
                        objectManager.createChild(eid, scene.add.circle(x + stepX, y + stepY, radius, col, 1));
                        objectManager.createChild(eid, scene.add.circle(x + stepX * 2, y + stepY * 2, radius, col, 1));
                        objectManager.createChild(eid, scene.add.circle(x + stepX * 3, y + stepY * 3, radius, col, 1));
        ...
        for (const eid of updatedProjectiles) {
            const x = Position.x[eid];
            const y = Position.y[eid];
            const bounds = new Phaser.Geom.Rectangle(0, 0, scene.scale.width, scene.scale.height);
...
function getEdgePoint(rect, centerPt, angle) {
    ...
    if (intersections.length > 0) {
        ...
        intersections.sort((a, b) => Phaser.Math.Distance.Between(centerX, centerY, a.x, a.y) -
