...
export class FiringIndicator {
    constructor(scene, getTargetId, radius = 60) {
        this.scene = scene;
        this.radius = radius;
    ...
    updateVector(angleDeg, power) {
        if (!this.line)
            return;
        const clampedPower = Phaser.Math.Clamp(power, 20, 100);
        const length = Phaser.Math.Linear(this.radius * 0.25, this.radius, (clampedPower - 20) / 80);
    ...
    handlePointerClick(pointer, onInput) {
        const { x, y } = getPosition(this.getTargetId());
        ...
        if (dist <= this.radius + 10) {
            ...
            const power = Phaser.Math.Clamp(Phaser.Math.Linear(20, 100, dist / this.radius), 20, 100);
            onInput(angle, power);
        }
    }
}
