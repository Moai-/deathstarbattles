export function getRandomEdgePosition(width, height, borderSize) {
    let x, y;
...
export function getRandomCenterPosition(width, height, centerSize) {
...
export function generateNonOverlappingPositions(width, height, radii, clearanceFn, existing = []) {
    const placed = [];
    for (const radius of radii) {
        ...
        if (!found) {
            throw new Error(`Failed to place object with radius ${radius} after 1000 attempts.`);
        }
    }
    return placed;
}
export const generateSupergiantStarPosition = (width, height, starRadius) => {
