...
export const playerClearance = (a, b) => a + b + 80;
export const objectClearance = (a, b) => a + b + 30;
const getPlayerRadius = (player) => getRadius(player.id);
export function runGameSetup(scene, world, config) {
    const { players, playerColors } = config;
    const width = scene.scale.width;
    const height = scene.scale.height;
    generateBackgroundStars(scene);
    const generateAndPlace = (generator, amount, existing = []) => {
        const generated = [];
        for (let i = 0; i < amount; i++) {
            generated.push(generator(world));
        }
        const radii = generated.map(getRadius);
        const positioned = generateNonOverlappingPositions(width, height, radii, objectClearance, existing);
    ...
    const planets = generateAndPlace(createRandomPlanet, planetCount, levelObjects);
    levelObjects = [...levelObjects, ...planets];
    const asteroidCount = Phaser.Math.Between(1, 5);
    const asteroids = generateAndPlace(createRandomAsteroid, asteroidCount, levelObjects);
    ...
    // 3. Place players
    const playerPositions = generateNonOverlappingPositions(width, height, parsedPlayers.map(getPlayerRadius), playerClearance, levelObjects);
